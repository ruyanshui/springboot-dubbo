...
import com.gaoxi.user.dao.IUserDao;
...
public class UserServiceImpl implements IUserService {
    ...
    private void checkParam(LoginReq loginReq) {
        ...
        if (StringUtils.isEmpty(loginReq.getPassword())) {
            throw new CommonBizException(ExpCodeEnum.PASSED_NULL);
        }
        // 手机，mail,用户名至少填一个
    ...
    public void batchUpdateUserState(BatchReq<UserStateReq> userStateReqs) {
        ...
        if (!userStateMap.isEmpty()) {
            for (Integer userSateCode : userStateMap.keySet()) {
                userDao.batchUpdateUserState(userSateCode, userStateMap.get(userSateCode));
            }
        }
    }
    ...
    @Override
    public void updateMenuOfRole(RoleMemuReq roleMenuReq) {
     ...
     * @param roleMenuReq
     */
    private void checkParam(RoleMemuReq roleMenuReq) {
    ...
    @Override
    public List<PermssionEntity> findPermissions() {
