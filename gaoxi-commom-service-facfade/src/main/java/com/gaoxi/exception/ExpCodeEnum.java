...
import static com.gaoxi.utils.ExpPrefixUtil.ComExpPredix;
import static com.gaoxi.utils.ExpPrefixUtil.ProdExpPrefix;
import static com.gaoxi.utils.ExpPrefixUtil.UserExpPrefix;
import static com.gaoxi.utils.ExpPrefixUtil.OrderExpPrefix;
import static com.gaoxi.utils.ExpPrefixUtil.AnlsExpPrefix;
/**
 * 全局的异常状态码和异常描述
 *
 */
public enum ExpCodeEnum implements Serializable{
    ...
    // 通用异常
    UNKNOW_ERROR(ComExpPredix + "000" , "未知接口"),
    ERROR_404(ComExpPredix + "001" , "没有该接口"),
    PARAM_NULL(ComExpPredix + "002", "参数为空"),
    NO_REPEAT(ComExpPredix + "003" , "请勿重复提交"),
    SESSION_NULL(ComExpPredix + "004" , "请求头中的SessionId不存在"),
    HTTP_REQ_METHOD_ERROR(ComExpPredix + "005" , "HTTP请求方法不正确 "),
    JSONERROR(ComExpPredix+ "006", "JSON解析异常"),

    // User模块异常
    USERNAME_NULL(UserExpPrefix + "000" , "用户名为空"),
    PASSED_NULL(UserExpPrefix + "001" , "密码为空"),
    AUTH_NULL(UserExpPrefix + "002" , "手机、电子邮件、用户名至少填一个" ),
    LOGIN_FAIL(UserExpPrefix + "003" , "登录失败"),
    UNLOGIN(UserExpPrefix + "004" , "尚未登录" ),
    NO_PREMISSION(UserExpPrefix + "005" , "没有权限"),
